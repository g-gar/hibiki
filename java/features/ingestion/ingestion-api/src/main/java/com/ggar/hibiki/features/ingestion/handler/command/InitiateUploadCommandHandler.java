...
public interface InitiateUploadCommandHandler
        extends CommandHandler<InitiateUploadCommandHandler.Initiate, UploadSession> {

    /**
     * Command to initiate a new upload session with one or more items.
     */
    record Initiate(UUID userId, List<ItemDescriptor> items) implements Command<UploadSession> {}
