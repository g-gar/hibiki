...
public class Id3v2Reader {
    ...
    public Mono<Id3v2Tag> readTag(Path filePath) {
        ...
        return Mono.using(
                ...
                channel -> parseTagFromBuffer(DataBufferUtils.readAsynchronousFileChannel(
                        () -> channel, 0, new DefaultDataBufferFactory(), 4096)),
                channel -> {
                    try {
                        channel.close();
                    } catch (IOException e) {
    ...
    private Mono<Id3v2Tag> parseTagFromBuffer(Flux<DataBuffer> buffers) {
        return DataBufferUtils.join(buffers).flatMap(buffer -> {
            try {
                ...
                if (majorVersion != 3) {
                    return Mono.error(new UnsupportedOperationException(
                            "Only ID3v2.3.0 is supported, found version 2." + majorVersion + "." + minorVersion));
                }
